#!/bin/bash
# ReSOT Ablation V3: GWOT only (No UOT, No SQ)
# 对应论文Table 2第三行: 仅GWOT结构感知重建，无UOT软分配和软量化
DATASET=${1:-"Instruments"}
DATA_ROOT=${2:-""}
DEVICE=${3:-"cuda:0"}
CKPT_DIR="checkpoints/resot_ablation/v3_${DATASET}"
...
python generate_ablation_index.py \
    ...
    --output_file "${DATASET}.ablation_v3_indices.json" \
    --content text \
    --embedding_file .emb-llama-td.npy \
    --data_root ${DATA_ROOT} \
    --use_gwot 1 \
