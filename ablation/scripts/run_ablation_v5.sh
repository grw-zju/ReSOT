#!/bin/bash
# ReSOT Ablation V5: GWOT+UOT (No SQ)
# 对应论文Table 2第五行: GWOT结构感知+UOT软分配，无软量化
DATASET=${1:-"Instruments"}
DATA_ROOT=${2:-""}
DEVICE=${3:-"cuda:0"}
CKPT_DIR="checkpoints/resot_ablation/v5_${DATASET}"
...
python generate_ablation_index.py \
    ...
    --output_file "${DATASET}.ablation_v5_indices.json" \
    --content text \
    --embedding_file .emb-llama-td.npy \
    --data_root ${DATA_ROOT} \
    --use_gwot 1 \
