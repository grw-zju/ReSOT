#!/bin/bash
# ReSOT Ablation V6: Full ReSOT (GWOT+UOT+SQ)
# 对应论文Table 2最后一行: 完整ReSOT，所有组件开启
DATASET=${1:-"Instruments"}
DATA_ROOT=${2:-""}
DEVICE=${3:-"cuda:0"}
CKPT_DIR="checkpoints/resot_ablation/v6_${DATASET}"
...
python generate_ablation_index.py \
    ...
    --output_file "${DATASET}.ablation_v6_indices.json" \
    --content text \
    --embedding_file .emb-llama-td.npy \
    --data_root ${DATA_ROOT} \
    --use_gwot 1 \
